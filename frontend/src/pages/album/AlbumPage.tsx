...
const AlbumPage = () => {
  ...
  console.log("🔥 AlbumPage is rendering");
  ...
  if (isLoading) return <div className="p-6 text-white">Loading album...</div>;
  if (!currentAlbum) return <div className="p-6 text-white">Album not found</div>;
  if (!currentAlbum.songs?.length) return <div className="p-6 text-white">⚠️ No songs in this album</div>;

  console.log("💿 currentAlbum:", currentAlbum);
  console.log("📀 Songs in album:", currentAlbum.songs);
        ...
        <div className="relative h-full">
          ...
          <div className="relative z-10">
            ...
            <div className="px-6 pb-4 items-center gap-6">
              <Button onClick={handlePlayAlbum} size="icon" className="w-14 h-14 rounded-full bg-green-500 hover:bg-green-600 hover:scale-105 transition-all">
                {isPlaying && currentAlbum.songs.some((song) => song._id === currentSong?._id) ? (
                  <Pause className="h-7 text-black" />
                ) : (
                  <Play className="h-7 text-black" />
                )}
              </Button>
            </div>
